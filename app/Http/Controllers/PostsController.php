...
class PostsController extends Controller
{
    ...
    public function addPost(Request $request){
       ...
       ->insert(['content' =>$content, 'user_id' => Auth::user()->id,
        'status' =>0, 'created_at' =>\Carbon\Carbon::now()->toDateTimeString(), 'updated_at' => \Carbon\Carbon::now()->toDateTimeString() ,'tfrom'=>$tfrom,'tto'=>$tto,'ttime'=>$ttime,'tdate'=>$tdate
         ]);
      if($createPost){
    ...
    public function updatePost($id, Request $request){
      ...
      $updatePost = DB::table('posts')->where('id',$id)->update([
         'content' => $request->updatedContent,
          'tfrom'=>$request->updatedfrom,
          'tto'=>$request->updatedto,
          'ttime'=>$request->updatedtime,
          'tdate'=>$request->updateddate
      ]);
      if($updatePost){
        return post::with('user','likes','comments')
        ->orderBy('created_at','DESC')->get();
